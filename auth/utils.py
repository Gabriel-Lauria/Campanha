...
def check_admin_permission(current_user = Depends(get_current_user)):
    if getattr(current_user, "permission", "user") != "admin":
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Acesso negado. Requer permissão de administrador.")
    return current_user